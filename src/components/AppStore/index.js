...
class AppStore extends Component {
  ...
  onChangeSearchApps = event => {
    this.setState({searchInput: event.target.value})
  }

  getFilterApps = searchResults => {
    const {activeTabId} = this.state
    const filterAppsList = searchResults.filter(
  ...
  render() {
    ...
    const filterApps = this.getFilterApps(searchResults)
      ...
      <div className="app-container">
        ...
        <div className="input-container">
          <input
            ...
            placeholder="search"
            onChange={this.onChangeSearchApps}
            value={searchInput}
          />
          <img
        ...
        <ul className="tabItem-container">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetails={eachTab}
              updateActiveTabDetails={this.updateTabId}
        ...
        <ul className="apps-container">
          {filterApps.map(eachApp => (
            <AppItem key={eachApp.appId} appsDetails={eachApp} />
          ))}
        </ul>
      </div>
    )
